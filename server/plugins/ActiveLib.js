...
import {ActiveBot} from "~/server/models/ActiveBot.schema";
const config = {
    number: 'BigNumber',
    precision: 20
}
...
export default defineNitroPlugin((nitroApp) => {
    ...
    nitroApp.ActiveBotsLib = {
        ...
        createBot: async function(data){
            ...
            await new ActiveBot(data).save()
        },
        // startBot: async function(id) {
        //     await ActiveBot.updateOne({ _id:id }, { status:true });
        // },
        // stopBot: async function(id) {
        //     await ActiveBot.updateOne({ _id:id }, { status:false });
        // },
        // deleteBot: async function(id) {
        //     await ActiveBot.findByIdAndDelete(id);
        // },
        // getBots: async function(){
        //     let bots = await ActiveBot.find({});
        //     let formattedBots = [];
        //
        ...
        //
        //         let profit = 0;
        //         let bots = await ActiveBot.find({id:bot._id});
        //         for (let i = 0; i < bots.length; i++) {
